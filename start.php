...
{
    elgg_register_route('mass_mail', ['path' => '/mass_mail/{segments}', 'resource' => 'mass_mail', 'requirements' => ['segments' => '.+'], 'defaults' => ['segments' => '']]);
...
function notifications_mass_mail_page_handler($segments)
{
    ...
    switch ($page) {
        case 'send':
            ...
            echo elgg_view_resource('mass_mail/send', ['container_guid' => $container_guid]);
            return true;
    }
    return false;
}
...
function notifications_mass_mail_get_subscriptions($hook, $type, $return, $params)
{
    ...
    if ($container instanceof ElggSite) {
        $recipients = new ElggBatch('elgg_get_entities', ['types' => 'user', 'callback' => false, 'limit' => 0]);
    } else {
        $recipients = new ElggBatch('elgg_get_entities', ['types' => 'user', 'relationship' => 'member', 'inverse_relationship' => true, 'relationship_guid' => $container->guid, 'callback' => false, 'limit' => 0]);
    }
...
function notifications_mass_mail_prepare_notification($hook, $type, $notification, $params)
{
    ...
    $template_params = array('action' => $action, 'actor' => $actor, 'object' => $object, 'target' => $target, 'recipient' => $notification->getRecipient(), 'sender' => $notification->getSender(), 'language' => $notification->language, 'site' => elgg_get_site_entity(), 'params' => $notification->params);
...
function notifications_mass_mail_page_menu_setup($hook, $type, $return, $params)
{
    if (elgg_in_context('admin')) {
        $return[] = ElggMenuItem::factory(['name' => 'mass_mail', 'parent_name' => 'administer_utilities', 'section' => 'administer', 'text' => elgg_echo('notifications:mass_mail'), 'href' => "mass_mail/send"]);
        ...
        if ($page_owner instanceof ElggGroup && elgg_get_plugin_setting('groups_mass_mail', 'notifications_mass_mail')) {
            $return[] = ElggMenuItem::factory(['name' => 'mass_mail', 'text' => elgg_echo('notifications:mass_mail:groups'), 'href' => "mass_mail/send/{$page_owner->guid}"]);
        }
    }
    return $return;
}